...
generate_correlated_data <- function(n = 100) {
  ...
  # Générer un tau de Kendall aléatoire entre -1 et 1
  tau <- runif(1, -1, 1)
  
  # Approximation de la corrélation de Pearson à partir du tau de Kendall
  rho <- sin(tau * pi / 2)
  
  # Générer deux variables normales standard indépendantes
  X <- rnorm(n)
  Z <- rnorm(n)
  
  # Construire Y avec la corrélation rho
...
find_max_N_power_test <- function(time_limit_sec, base = 10, exp_min = 6, exp_max = 10) {
  ...
  for (method in names(methods)) {
    ...
    for (exp in exp_min:exp_max) {
      N_test <- base^exp  # Puissance de 10 de N_test
  ...
  cat("\nTemps total d'exécution de la fonction: ", total_exec_time_min, " minutes\n")
  
  return(N_results)
}

max_N_test_results <- find_max_N_power_test(time_limit_sec = 10, base = 2, exp_min = 14, exp_max = 20)
